...
function getPhotosetImages() {
	$(".post_thumb .photoset").each(function () {
		...
		$.getJSON("/api/read/json?id="+ID+"&callback=?", {}, function (D) {
			photos = D["posts"][0]["photos"];
			for(i = 0; i<photos.length; i++) {
				thumb = photos[i]["photo-url-250"];
...
if (embedCode.indexOf("iframe") >= 0) {
	...
	if (swf.indexOf("youtube.com") >= 0) {        
		...
		thumb_url = "http://i.ytimg.com/vi/"+youtube_id+"/0.jpg";
		$("#thumb_"+id+" .video").prepend("<div class=\"bg rounded\" style=\"background-image:url('"+thumb_url+"');\"></div>");
...
function history(hash) {
	...
	if (id === "/") {
		...
		// thanks to Brian Long ‰ÛÓ http://newobj.net/
...
function check_for_more() {
	if (view === "list") { 
		...
		//$("#debug").text(yPos+" / "+h)
		if ((yPos/(h-wH)) >= 0.8 && !is_loading_next && next_page !== "no") {
			...
			$("#temp").load(next_page, {}, function (d) {
				...
				window.setTimeout(can_check_for_more_now, 2000);
			});
		}
	}
}
...
//
function add_credit() {
	$("html body").append("<div id=\"credit\"><a href=\"http://cargotheme.tumblr.com\">Cargo Theme</a> improved by <a href=\"http://tumblr.mattclegg.com\">Matt Clegg</a>.</div>");
}
...
function init () {
	...
	$(".post_thumb").hover(function () {
		$('#title_text').html($(this).find(".hover_text").html()).show('slow');
	},function () {
		$('#title_text').hide();
	});
